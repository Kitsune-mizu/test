...
export async function GET() {
  try {
    ...
    const ordersChartData = Object.entries(ordersByDate).map(([date, count]) => ({
      date,
      orders: count,
    }))
    ...
    const topProductsData = productsData?.map((item: any) => ({
    ...
    const customersChartData = Object.entries(customersByDate).map(([date, count]) => ({
      date,
      customers: count,
    }))
    ...
    return NextResponse.json({
      ...
      totalRevenue: revenueData?.reduce((sum: number, order: any) => sum + (order.total_price || 0), 0) || 0,
